...
<script>
function showHeader() {
    var DivId = document.getElementById("sample-table");
...
for(i in 1:nrow(summary.dt)){
  ...
  if(grepl("expression",summary.dt$type[i])){
    for(val in ExpSequences){
      if(grepl(val,summary.dt$type[i])){
        ...
        forAction=paste0("showImg('",g,".expr');showImg('",g,".gg')")
  ...
  if(grepl("amp|hetdel",summary.dt$type[i])){
    for(val in CnvSequences){
      if(grepl(val,summary.dt$type[i])){
        summary.dt$type[i]=gsub(val,basebutton,summary.dt$type[i])
        summary.dt$type[i]=gsub("thisValue",val,summary.dt$type[i])
        forAction=paste0("showImg('",g,".cnv')")
        summary.dt$type[i]=gsub('openTabset',forAction,summary.dt$type[i])
      }
    }
  }
  if(grepl("fusion",summary.dt$type[i])){
        ...
        forAction=paste0("showImg('",fusions.gallery.dt[driver.name == g, driver.name],".fus')")
        summary.dt$type[i]=gsub('openTabset',forAction,summary.dt$type[i])
  }
  ...
  if(grepl("missense",summary.dt$type[i])){
        ...
        forAction=paste0("showImg('",fusions.gallery.dt[driver.name == g, driver.name],".snvt')",";showHeader(snvheader)")
        summary.dt$type[i]=gsub('openTabset',forAction,summary.dt$type[i])
  }
  summary.dt$type[i]=gsub(", ,",",",summary.dt$type[i])
}
...
```{r surface-cn-driver, dropBelow=TRUE, results = "asis", echo = FALSE, out.width= "100%", fig.align = "center", max.height = 800}
cn.gallery.dt = fread(file.path(params$outdir, "cn.gallery.txt"))
if (nrow(cn.gallery.dt) > 0) {
    for (g in cn.gallery.dt[, gene_name]) {
        cat(
...
```{r, SNV-driver, results = "asis", echo = FALSE}
...
for(i in 1:nrow(driver.mutations.dt)){
  ...
  for(j in 1:ncol(driver.mutations.dt)){
    thisL=paste0('<td>',as.vector(unlist(as.vector(unlist(driver.mutations.dt[i,colnames(driver.mutations.dt)[j]])))),'</td>')
...
```
 ...
 <table class="display dataTable no-footer table table-condensed" aria-describedby="DataTables_Table_0_info">
  <tr id="sheader" style="display:none"><th>Species</th><th>Genre</th></tr>
   <tr class="odd" id="tsetosa" style="display:none"><td>Setosa</td><td >Iris</td></tr>
   <tr class="even" id="tversicolor" style="display:none"><td>Versicolor</td><td>Iris</td></tr>
</table>
