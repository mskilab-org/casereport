...
```{r, summ-driver, dropAboveExp=TRUE, results = "asis", echo = FALSE}
...
CnvSequences=c("amp","hetdel","loh","homdel")
...
for(i in 1:nrow(summary.dt)){
  ...
  if(grepl("expression",summary.dt$type[i])){
    for(val in ExpSequences){
      if(grepl(val,summary.dt$type[i])){
        ...
        forAction=paste0("showImg('",g,".expr');showImg('",g,".gg')",";showImg('",g,".expt')",";showHeader('expheader','divEXP')")
        summary.dt$type[i]=gsub('openTabset',forAction,summary.dt$type[i])
      }
    }
  }
  if(grepl("amp|hetdel|homdel|loh",summary.dt$type[i])){
    for(val in CnvSequences){
      if(grepl(val,summary.dt$type[i])){
        summary.dt$type[i]=gsub(val,basebutton,summary.dt$type[i])
        summary.dt$type[i]=gsub("thisValue",val,summary.dt$type[i])
        forAction=paste0("showImg('",g,".cnv')",";showImg('",g,".cnvtt')",";showHeader('cnvheader','divCNV')")
        summary.dt$type[i]=gsub('openTabset',forAction,summary.dt$type[i])
      }
    }
  }
  if(grepl("fusion",summary.dt$type[i])){
        ...
        forAction=paste0("showImg('",fusions.gallery.dt[driver.name == g, driver.name],".fus')",";showImg('",g,".fust')",";showHeader('fusheader','divFUS')")
        summary.dt$type[i]=gsub('openTabset',forAction,summary.dt$type[i])
  }
  ...
  if(grepl("missense",summary.dt$type[i])){
        ...
        forAction=paste0("showImg('",driver.mutations.dt[gene == g, gene],".snvt')",";showHeader('snvheader','divSNV')")
        summary.dt$type[i]=gsub("openTabset",forAction,summary.dt$type[i])
  }
...
<input type="button" value="Clear All" id="CLBut" onclick="hideImgs()" />

```{r, cnv-driver-table, results = "asis", echo = FALSE}
...
cat('<div class="divScroll" id="divCNV" style="display:none">')
...
```{r, expr-driver-table, results = "asis", echo = FALSE}
...
if(nrow(driver.genes.expr)>0){
  cat('<div class="divScroll" id="divEXP" style="display:none">')
...
```{r, fus-driver-table, results = "asis", echo = FALSE}
...
if(any(fusions.gallery.dt$genes %in% driverGenes)){
  cat('<div class="divScroll" id="divFUS" style="display:none">')
  ...
  for(i in 1:nrow(driver.genes.expr)){
   if(driver.fusions.dt$driver.name[i] %in% driverGenes){
    for(j in 1:ncol(dat)){
...
```{r, fus-driver, results = "asis", echo = FALSE}
...
```{r, prox-driver, results = "asis", echo = FALSE}
...
```{r, SNV-driver, dropBelow=TRUE, results = "asis", echo = FALSE}
if(nrow(driver.mutations.dt)>0){
...
cat('<div class="divScroll" id="divSNV" style="display:none">')
...
```{r, summ-surface, dropAboveExp=TRUE, results = "asis", echo = FALSE}
  ...
  if(grepl("expression",summary.dt$type[i])){
    for(val in ExpSequences){
      if(grepl(val,summary.dt$type[i])){
        summary.dt$type[i]=gsub(val,basebutton,summary.dt$type[i])
        summary.dt$type[i]=gsub("thisValue",val,summary.dt$type[i])
        forAction=paste0("showImg('",g,".expr');showImg('",g,".gg')",";showImg('",g,".sexpt')",";showHeaderSurface('sexpheader','SdivEXP')")
        summary.dt$type[i]=gsub('openTabset',forAction,summary.dt$type[i])
      }
    }
  }
  if(grepl("amp|hetdel|homdel|loh",summary.dt$type[i])){
    for(val in CnvSequences){
      if(grepl(val,summary.dt$type[i])){
        summary.dt$type[i]=gsub(val,basebutton,summary.dt$type[i])
        summary.dt$type[i]=gsub("thisValue",val,summary.dt$type[i])
        forAction=paste0("showImg('",g,".cnv')",";showImg('",g,".scnvtt')",";showHeaderSurface('scnvheader','SdivCNV')")
        summary.dt$type[i]=gsub('openTabset',forAction,summary.dt$type[i])
      }
    }
  }
    if(grepl("fusion",summary.dt$type[i])){
        ...
        forAction=paste0("showImg('",fusions.gallery.dt[driver.name == g, driver.name],".fus')",";showImg('",g,".fust')",";showHeaderSurface('sfusheader','SdivFUS')")
        summary.dt$type[i]=gsub('openTabset',forAction,summary.dt$type[i])
  }
  summary.dt$type[i]=gsub(", ,",",",summary.dt$type[i])
}
...
<input type="button" value="Clear All" id="SCLBut" onclick="hideImgsSurface()" />

```{r, cnv-surface-table, results = "asis", echo = FALSE}
...
if(nrow(surface.genes.cnv)>1){
cat('<div class="divScroll" id="SdivCNV" style="display:none">')
...
for(i in 1:nrow(surface.genes.cnv)){
  ...
  for(j in 1:ncol(driver.genes.cnv)){
    thisL=paste0('<td>',as.vector(as.matrix(surface.genes.cnv)),'</td>')
    rLine=paste0(rLine,thisL)
  }
  cat(paste0(rLine,'</tr>'))
}
...
```{r surface-cn-driver, results = "asis", echo = FALSE, out.width= "100%", fig.align = "center", max.height = 800}
cn.gallery.dt = fread(file.path(params$outdir, "surface.cn.gallery.txt"))
if (nrow(cn.gallery.dt) > 0) {
    for (g in cn.gallery.dt[, gene_name]) {
        cat(
...
```{r, expr-surface-table, results = "asis", echo = FALSE}
...
if(nrow(surface.genes.expr)>0){
  cat('<div class="divScroll" id="SdivEXP" style="display:none">')
...
```{r, expr-surface, results = "asis", echo = FALSE}
expr.hist.dt = fread(file.path(params$outdir, "surface.expr.histograms.txt"))
...
```{r, fus-surface-table, results = "asis", echo = FALSE}
driver.fusions.fname = file.path(params$outdir, "fusions.driver.txt")
if (file.exists(driver.fusions.fname)) {
   driver.fusions.dt = fread(driver.fusions.fname)
   if (nrow(driver.fusions.dt)) {
...
if(any(fusions.gallery.dt$driver.name %in% driverGenes)){
  cat('<div class="divScroll" id="SdivFUS" style="display:none">')
  ...
  for(i in 1:nrow(fusions.gallery.dt)){
...
```{r, fus-surface, dropBelow=TRUE, results = "asis", echo = FALSE}
if (nrow(fusions.gallery.dt) > 0) {
        for (g in fusions.gallery.dt[, genes]) {
          if(!(g %in% driverGenes)){
            cat(
...
knitr::include_graphics(file.path(params$outdir, "wgs.gtrack.png"),rel_path = getOption("knitr.graphics.rel_path", FALSE))
```
</div>
</div>
...
knitr::include_graphics(file.path(params$outdir, "wgs.circos.png"),rel_path = getOption("knitr.graphics.rel_path", FALSE))
```
</div>
...
```{r, surface-cnv-table, dropAboveExp=TRUE, results = "asis", max.width= "100%"}
surface.genes.cnv = fread(file.path(params$outdir, "surface.genes.cnv.txt"))
surface.genes.cnv$min_normalized_cn=unlist(lapply(surface.genes.cnv$min_normalized_cn,FUN=signif,digits=4))
surface.genes.cnv$max_normalized_cn=unlist(lapply(surface.genes.cnv$max_normalized_cn,FUN=signif,digits=4))
surface.genes.cnv$expr.quantile=unlist(lapply(surface.genes.cnv$expr.quantile,FUN=signif,digits=4))
...
```{r, surface-cn-tabset, dropBelow=TRUE, results = "asis", echo = FALSE, out.width= "100%", fig.align = "center", max.height = 800}
cn.gallery.dt = fread(file.path(params$outdir, "surface.cn.gallery.txt"))
if (nrow(cn.gallery.dt) > 0) {
    cat("\n")
    for (g in cn.gallery.dt[, gene_name]) {
...
```{r, surface-expr-table,  dropAboveExp=TRUE, results = "asis", max.width= "100%"}
surface.genes.expr = fread(file.path(params$outdir, "surface.genes.expr.txt"))
if (surface.genes.expr[,.N] > 0){
    surface.genes.expr$min_normalized_cn=unlist(lapply(surface.genes.expr$min_normalized_cn,FUN=signif,digits=4))
    surface.genes.expr$max_normalized_cn=unlist(lapply(surface.genes.expr$max_normalized_cn,FUN=signif,digits=4))
...
```{r, surface-waterfall, results = "asis", max.width = "100%"}
...
```{r surface_expression, dropBelow=TRUE, results = "asis", echo = FALSE, out.width= "100%", fig.align = "center", max.height = 800}
...
knitr::include_graphics(file.path(params$outdir, "ridgeplot.png"),rel_path = getOption("knitr.graphics.rel_path", FALSE))
```
...
```{r, driver-germline-snv-indel, dropAboveExp=TRUE, dropBelow=TRUE, results = "asis", echo = FALSE, out.width = "100%", fig.align = "center"}
...
if (file.good(paste0(params$outdir, "/deconstruct_sigs.png"))){
    ...
    cat("\n")
    ## knitr::include_graphics(paste0(params$outdir, "/deconstruct_sigs.png"),rel_path = getOption("knitr.graphics.rel_path", FALSE))
...
knitr::include_graphics(file.path(params$outdir, "cn.pp.png"),rel_path = getOption("knitr.graphics.rel_path", FALSE))
```
...
if (file.good(file.path(params$outdir, "allele.scatter.png"))){
    knitr::include_graphics(file.path(params$outdir, "allele.scatter.png"),rel_path = getOption("knitr.graphics.rel_path", FALSE))
}
```
