  ...
  if(grepl("expression",summary.dt$type[i])){
    for(val in ExpSequences){
      if(grepl(val,summary.dt$type[i])){
        summary.dt$type[i]=str_replace(val,basebutton,summary.dt$type[i])
        summary.dt$type[i]=str_replace("thisValue",val,summary.dt$type[i])
      }
    }
  }else if(grepl("amp|hetdel",summary.dt$type[i])){
...
```{r}
wol=c("underexpression","overexpression")
grepl("hover|hunder",wol)

library(stringr)
sample="wololo/gene/name"
gsub(".*gene/","",sample)
```
