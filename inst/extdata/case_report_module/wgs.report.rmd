...
if (nrow(summary.dt) == 0){
  cat("no genes found in summary dataframe.")
}else{
...
if (nrow(driver.genes.expr) > 0){
  ...
  cat("No driver genes expressions found")
...
if(any(fusions.gallery.dt$genes %in% driverGenes)){
  if (nrow(driver.genes.expr) >0){
    ...
    cat("no driver genes expression found")}
  cat('</div>')
}
```
...
if (nrow(driver.mutations.dt) > 0){
  ...
  cat('no driver mutations found. empty driver mutations table.')
}
```
***
...
SnvSequences=c("missense")
if (nrow(summary.dt) == 0){
  cat("empty summary surface table.")
}else{
  for(i in 1:nrow(summary.dt)){
    g=gsub(".*/gene/","",summary.dt$gene[i])
    g=gsub(" target.*$","",g)
...
if (nrow(surface.genes.cnv) >0){
  ...
  cat('no surface cnv genes found. \n')
...
if (nrow(surface.genes.expr)>0){
  ...
  cat("no surface gene expressions found. \n")
...
if(any(fusions.gallery.dt$driver.name %in% driverGenes)){
  ...
  if (nrow(driver.genes.expr) > 0){
    ...
    cat("no values in driver.genes.expression table")
  }
...
knitr::include_graphics(normalizePath(file.path(params$outdir, "wgs.gtrack.png")), 
                        error = FALSE)
```
</div>
</div>
...
knitr::include_graphics(normalizePath(file.path(params$outdir, "wgs.circos.png")),
                        error = FALSE)
```
</div>
