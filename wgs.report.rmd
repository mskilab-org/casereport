...
#This is the first function for the buttons, which opens the section that will expand/collapse for each button. This function should be run in a chunk immediately after the section header (title with a single #). IF THERE IS A SECTION WITHOUT A CHUNK BEFORE THE FIRST SUBSECTION (title with two or more #) THEN ADD AN EMPTY CHUNK AFTER THE SECTION TITLE AND CALL THIS HOOK (see Whole Genome View for an example).

#The function has two variants. For section which will be collapsed by default, use this one.
...
#This is the second function for the buttons, which closes the section that will expand/collapse for each button. It should be called in the last chunk of the section.  FOR SECTIONS (whole sections, i.e: single # before name) WITH TABSET ADD AN ADDITIONAL </div> AFTER THE LAST CHUNK.
...
```{r, param-lead, dropAboveExp=TRUE, results = "hide", echo = FALSE}
```
...
```{r, wgs-lead, dropAboveExp=TRUE, results = "hide", echo = FALSE}
```
...
```{r, driver-cnv-table, dropAboveCol=TRUE, results = "asis", max.width= "100%"}
...
```{r, fus-lead, dropAboveCol=TRUE, results = "hide", echo = FALSE}
```
...
```{r, driver-expr-table,  dropAboveCol=TRUE, results = "asis", max.width= "100%"}
 ...
 if (file.good(paste0(params$outdir, "/", "rna.change.txt"))) {
	...
	if (file.exists(expr.png.fn) & file.exists(gg.png.fn)) {
         cat("###", cool.exp[gene == g, paste0(gene, " (", direction, ")", "<br>", "\n")])
       	 cat("Z-Score:",cool.exp[cool.exp$gene==g,]$zscore)
...
```{r, SV-lead, dropAboveCol=TRUE, results = "hide", echo = FALSE}
```
...
```{r, driver-snv-indel, dropAboveCol=TRUE, dropBelow=TRUE, results = "asis", echo = FALSE, out.width = "100%", fig.align = "center"}
...
```{r, Oneness_twoness, dropAboveCol=TRUE, dropBelow=TRUE, results = "asis", echo = FALSE, out.width = "100%", fig.align = "center"}
...
```{r, HRDetect, dropAboveCol=TRUE,  dropBelow=TRUE, results = "asis", echo = FALSE, out.width="100%", fig.align = "center", max.height = 800}
...
```{r, enhancer-hijacking, dropAboveCol=TRUE, dropBelow=TRUE, results = "asis", echo = FALSE, out.width="100%", fig.align = "center"}
...
```{r ppfit, dropAboveCol=TRUE, results = "asis", echo = FALSE, out.width="100%", fig.align = "center"}
