...
ifelse(grepl("^[aeiou]", ignore.case = T), "an", "a")` `r params$tumor_type`
---
...
This sample shows the following known structural drivers:
...
driver.fusions.dt = fread(file.path(opt$outdir, "fusions.driver.txt"))
driver.fusions.dt[, .(name, walk.id, numchr, total.aa, gene.pc, ev.id, ev.type)] %>% kbl() %>% kable_styling()
```
...
other.fusions.dt = fread(file.path(opt$outdir, "fusions.other.txt"))
other.fusions.dt[, .(name, walk.id, numchr, total.aa, gene.pc, ev.id, ev.type)] %>% kbl() %>% kable_styling()
```
...
knitr::include_graphics(file.path(opt$outdir, "ridgeplot.png"))
```
...
sv.slickr.dt = fread(file.path(opt$outdir, "sv.gallery.txt"))[!is.na(plot.link),]
