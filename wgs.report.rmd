...
`r format(round(summary.list$cna_frac * 100, 2), nsmall = 2)`% of the genome shows copy number alterations (CNA), defined as CN at most `r params$del_thresh` times ploidy (deletions) or at least `r params$amp_thresh` times ploidy (amplifications). 

The total number of somatic SNV/INDELs genome-wide is `r format(round(summary.list$mut_count, 0), nsmall = 0)`. The **Tumor Mutation Burden (TMB)** is **`r format(round(summary.list$mut_per_mbp, 2), 2)` per Mbp**. The total **junction burden** is **`r length(jj)`**.
...
if (file.exists(paste0(params$outdir, "/cool.expr.rds"))){
    ...
    for (g in cool.exp[order(qt, decreasing = T), gene]){
        ...
        cat()
        cat(
            paste0("![](", paste0(params$outdir, cool.exp[g, paste0("/", gene, ".", direction, ".expr.png")]),")"), "\n"
        )
        cat("\n")
        cat(
            paste0("![](", expr.slickr.dt[gene_name == g, plot.fname],")"), "\n"
        )
        ## cat(
        ## 	paste0(
        ## 		'<p float="left">',
