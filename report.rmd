...
som = dt2gr(som.dt)
som.gt = gTrack(som, name = "coding variants", circles = TRUE, bars = TRUE)
```
...
***
# Nonsynonymous Mutations and INDELs {.tabset .tabset-fade .tabset-pills}
...
if (file.exists(paste0(params$outdir, "/fusions.rds"))){
    ...
    plotsToSVG <- lapply(seq_along(fu), function(i){
        svglite::xmlSVG(
            code = {
                fname = fu$dt[i, name] %>% gsub(pattern = ",", replacement = "--")
                fp = grbind(
                ...
                plot(c(cvgt, gg$gt, fu$gtrack(name = )), fp, legend.params = list(plot = FALSE))
                title(fname)
            },
            standalone = TRUE)
    })
